...
router.post("/", async (req, res, next) => {
    ...
    // 오류가 날 수도 있기 때문에 try, catch 문 작성
    try {

        // 게시글을 생성하기전에 작성자를 같이 담아주기 위해서 email에 맞는 정보를 가지고 있는,
...
router.get("/", async (req, res, next) => {
    ...
    // req.query page라는 쿼리를 가져오는데 이게 undefind이면 1을 넣어라
    ...
    // perPage는 1페이지에 총 보여지는 게시글의 개수
    let perPage = Number(req.query.perPage) || 6; // 한 페이지에 보여지는 게시글의 개수
